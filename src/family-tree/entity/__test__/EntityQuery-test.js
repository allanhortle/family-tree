...
const EntityQueryModule = proxyquire('../EntityQuery', {
    'family-tree/util/request': function(url, options) {
...
process.env.FAMILY_TREE_GRAPHQL_SERVER = 'http://localhost:3000';
...
test('EntityQuery Throws error if FAMILY_TREE_GRAPHQL_SERVER env var isn\'t set', tt => {
    process.env.FAMILY_TREE_GRAPHQL_SERVER = '';
    tt.throws(
        () => requestActionThunk(()=>{}),
